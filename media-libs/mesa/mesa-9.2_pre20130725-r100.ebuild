...
IUSE="${IUSE_VIDEO_CARDS}
	bindist +classic debug +egl +gallium gbm gles1 gles2 +opengl +llvm +nptl opencl
...
DEPEND="${RDEPEND}
	...
	opengl? ( >=x11-proto/glproto-1.4.15-r1 )
...
src_prepare() {
	...
	epatch "${FILESDIR}"/${PN}-9.1-optional-gl-headers.patch
	epatch "${FILESDIR}"/${PN}-9.2-allow-gles-only.patch
...
src_install() {
	...
	if use !opengl; then
		touch "${ED}/usr/$(get_libdir)/opengl/${OPENGL_DIR}/.gles-only"
	fi
}
